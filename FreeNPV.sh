...
if [ $useSavedVPNlist -eq 0 ];then
    ...
    curl -s $cURLproxyString http://202.5.221.90:19927/api/iphone/ > $vpnList
elif [ ! -s $vpnList ];then
    echo 'Getting the VPN list'
    curl -s $cURLproxyString http://202.5.221.90:19927/api/iphone/ > $vpnList  
else
...
CreateVPNConfig () {
    ...
    if [ $useFirstServer -eq 1 ]; then
        index=0
        #echo ${array[$index]} | awk -F "," '{ print $15 }' | base64 -d > /tmp/openvpn3/config.txt
        echo ${array[$index]} | awk -F "," '{ print $15 }' | base64 -d > config
    else       
        index=$(($RANDOM % $size))
        echo ${array[$index]} | awk -F "," '{ print $15 }' | base64 -d > config
    fi
...
while true
    do
        ...
        read -p "Поменять сервер? y/n?: " confirm && [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]] || exit
    done